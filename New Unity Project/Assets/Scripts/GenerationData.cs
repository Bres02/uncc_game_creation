...
public class GenerationData : ScriptableObject // Creates and Modifies a 2D array of integers
{
    ...
    // tileValue: the value that is added to the data array
    {
        ...
        for (int i = 0; i < size; i++)
        {
            tempData[i, 0] = (int)Random.Range(1, borderVariability + 1);
            tempData[i, size - 1] = (int)Random.Range(1, borderVariability + 1);
            tempData[0, i] = (int)Random.Range(1, borderVariability + 1);
            tempData[size - 1, i] = (int)Random.Range(1, borderVariability + 1);
        }
    ...
    // tileValue: the value that is added to the data array
    {
        ...
        while (pathCounter < positionCount + 1)
        {
            ...
            int iTest = (int)Random.Range(pathDistance, size - pathDistance);
            int jTest = (int)Random.Range(pathDistance, size - pathDistance);
        ...
        {
            int destination = (int)Random.Range(k + 1, positionCount + 1); // Picks a loctation to make a path to
        ...
        for (int k = 0; k < pathSize + 1; k++)
        {
            for (int i = 0; i < size; i++)
            {
                for (int j = 0; j < size; j++)
                {
                    surroundingMin(i, j, tempData[i, j] - (int)Random.Range(0, spreadValue)); // Increases the surrounding tiles, with the amount depending on spreadValue
                }
            }
        }
        ...
        for (int i = 0; i < size; i++)
        {
            for (int j = 0; j < size; j++)
            {
                if (tempData[i, j] <= decayThreshhold && (int)Random.Range(0, decayValue) == 0) tempData[i, j] = 0;
            }
        }
        // Adds the tileValue to data wherever tempData has a value > 0
        for (int i = 0; i < size; i++)
    ...
    // tileValue: the value that is added to the data array
    {
        ...
        do
        {
            int rand = (int)Random.Range(0, iPos.Count); // Picks a random location
    ...
    public void resetData() // Changes all of the values in data to 0
    {
        for (int i = 0; i < size; i++)
        {
            for (int j = 0; j < size; j++)
